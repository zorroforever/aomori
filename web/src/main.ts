...
app.innerHTML = `
  <header class="topbar">
    ...
    <div class="connection"><span id="statusDot" class="dot offline"></span><span id="statusText" role="status" aria-live="polite">未连接</span><button id="connectBtn" class="button">连接节点</button></div>
  </header>
  <main class="layout">
    <aside class="sidebar">
...
function syncCommandControls() {
  document.querySelectorAll<HTMLButtonElement>('[data-command], [data-direction]').forEach(button => { button.disabled = state.commanding; });
  document.querySelectorAll<HTMLSelectElement>('.transfer-target').forEach(select => { select.disabled = state.commanding; });
}
function setCommandBusy(busy: boolean) {
  state.commanding = busy;
  ($('commandInput') as HTMLInputElement).disabled = busy;
...
async function refreshStatus() { const result = await rpc('aomori_get_quests', { actor_id: state.actor }); const quests = result.quests || []; state.quests = quests; $('questList').innerHTML = quests.length ? quests.map((quest: any) => `<div class="quest-item"><div class="quest-head"><strong>${escapeHtml(quest.title)}</strong><span class="quest-status ${escapeHtml(quest.status)}">${escapeHtml(quest.status)}</span></div><dl><div><dt>目标</dt><dd>${escapeHtml(quest.required_item)}</dd></div><div><dt>交付</dt><dd>${escapeHtml(quest.completion_zone)}</dd></div><div><dt>奖励</dt><dd>${quest.reward_balance} coins</dd></div>${quest.prerequisite_quest_ids?.length ? `<div><dt>前置</dt><dd>${quest.prerequisite_quest_ids.map((id: string) => escapeHtml(id)).join(', ')}</dd></div>` : ''}</dl>${quest.status === 'accepted' ? `<button class="task-button" data-command="complete ${escapeHtml(quest.id)}">完成任务</button>` : ''}</div>`).join('') : '<span class="muted">暂无任务</span>'; syncCommandControls(); const actor = await rpc('aomori_get_entity', { entity_id: state.actor }); const account = actor?.owner ? await rpc('aomori_get_account', { name: actor.owner }) : null; $('balance').textContent = String(account?.balance || 0); }
...
async function command(raw: string) {
  ...
  if (!parts[0]) return;
  if (parts[0] === 'look') return await look();
