...
When the Web client creates an identity, it generates an Ed25519 key pair locally, sends only the public key to `aomori_create_account`, and stores the secret key in browser `localStorage` under the RPC URL and account name. The administrator token is used only for the two creation requests and is not persisted.

The client can export a versioned encrypted JSON backup. It derives a 256-bit key from a password using PBKDF2-SHA-256 with 210,000 iterations and encrypts the Ed25519 secret key with NaCl secretbox. Import verifies the backup public key and checks it against the account public key returned by the selected node before storing the secret key. Passwords must contain at least eight characters and are never persisted.

Clearing browser storage without an exported backup loses the local secret key. If concurrent browser actions race on the same account nonce, the client fetches the latest nonce, re-signs, and retries once.
