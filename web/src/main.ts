...
function connectEvents() { if (state.socket) { state.socket.onclose = null; state.socket.close(); } state.socket = new WebSocket(state.rpc.replace(/^http/, 'ws') + '/events'); state.socket.onopen = () => { setStatus('online', '节点在线'); addLog('实时事件通道已连接', 'system'); recoverEvents().catch(error => addLog(`事件补偿失败: ${error.message}`, 'error')); }; state.socket.onmessage = event => { const value = JSON.parse(event.data) as WorldEvent | EventStreamLagged; if (isLagMessage(value)) { addLog(`事件流落后 ${value.missed} 条，正在补偿`, 'error'); recoverEvents().catch(error => addLog(`事件补偿失败: ${error.message}`, 'error')); } else renderEvent(value); }; state.socket.onclose = () => { setStatus('connecting', '事件通道重连中'); addLog('实时事件通道已断开，准备重连', 'error'); window.clearTimeout(state.reconnectTimer); state.reconnectTimer = window.setTimeout(connectEvents, 3000); }; }
